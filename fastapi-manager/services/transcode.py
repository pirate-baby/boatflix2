...
# Import remote transcode module
try:
    from services import remote_transcode
    REMOTE_TRANSCODE_AVAILABLE = True
except ImportError:
    REMOTE_TRANSCODE_AVAILABLE = False
    logger.warning("Remote transcode module not available")
...
async def transcode_video(
    ...
    Args:
        ...
        use_remote: Use remote transcoding (None = auto-detect from settings)
    ...
    # Determine if we should use remote transcoding
    if use_remote is None:
        use_remote = settings.REMOTE_TRANSCODE_ENABLED and REMOTE_TRANSCODE_AVAILABLE
    ...
    # If remote transcoding is enabled, delegate to remote service
    if use_remote:
        _append_log(f"  Using remote transcode on {settings.REMOTE_TRANSCODE_HOST}")

        try:
            result = await remote_transcode.remote_transcode_video(
                ...
                progress_callback=lambda msg: _append_log(f"  Remote: {msg}"),
            )

            if not result.get("success"):
                _append_log(f"Remote transcode failed: {result.get('error')}")
                _append_log("Falling back to local transcoding...")
                # Fall through to local transcoding
                use_remote = False
            else:
                # Remote transcode succeeded - continue with post-processing
                ...
                _append_log(f"Remote transcode complete: {video_path.name} "
                ...
                history.append({
                    ...
                    "duration_seconds": 0,  # Remote doesn't track duration yet
            ...
            _append_log(f"Remote transcode exception: {e}")
            _append_log("Falling back to local transcoding...")
            use_remote = False

    # Local transcoding (original code)
    if not use_remote:
